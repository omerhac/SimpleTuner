...
class FluxPipeline(DiffusionPipeline, SD3LoraLoaderMixin):
    ...
    @staticmethod
    def _pack_latents(latents, masked_image_latents, clothes_latents, masks, batch_size, num_channels_latents, height, width):
    ...
    def prepare_latents(
        ...
        generator,
        latents=None,
        encoded_masked_image=None,
        endode_cloth_image=None,
        mask=None
    ):
        ...
        prep_mask = self.prepare_mask(mask, latents)
        latents = self._pack_latents(
            latents, encoded_masked_image, endode_cloth_image, prep_mask, batch_size, num_channels_latents, height, width
        )
    ...
    def __call__(
        ...
        cloth_image: Optional[torch.FloatTensor] = None,
        ...
        encoded_masked_image = self.vae.encode(masked_image.to(self.vae.device, dtype=self.vae.dtype), return_dict=False)[0].sample()
        encoded_cloth_image = self.vae.encode(cloth_image.to(self.vae.device, dtype=self.vae.dtype), return_dict=False)[0].sample()
